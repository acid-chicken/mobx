#!/usr/bin/env node
/* Publish.js, publish a new version of the npm package as found in the current
 * directory */
...
function writeJSON(path, obj) {
  return new Promise((resolve, reject) => {fs.writeFile(
                         path, JSON.stringify(obj, null, 2), err => {
                           if (err) {
                             reject(err)
                           } else {
                             resolve()
...
async function main() {
      ...
      [
        {
          ...
          choices : [
            {title : "Nothing, abort this", value : null}, {
              title : `Patch version (${maskVerWithX(nextPatch)})`,
              value : nextPatch
            },
            {
              title : `Minor version (${maskVerWithX(nextMinor)})`,
              value : nextMinor
            },
        ...
        },
        {
          ...
          name : "gitUser",
          message :
              "Enter the github user to publish the docs with. Empty to skip.",
          initial : gitUser
        }
      ],
      {onCancel : () => exit(0)})
  ...
  const npmInfoRet = run(`npm info ${rootPkg.name} --json`,
                         {continueOnErrors : true, silent : true})
  ...
  if (npmInfoRet.code === 0) {
    ...
    run(`git commit --no-verify -m "Published version ${
        maskVerWithX(resp.action)}"`)
    run("git push")

    versions.forEach(ver => {run(`git tag ${ver}`)})
    run("git push --tags")
